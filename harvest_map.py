...
PALETTE = {
    STATE_DEPLOY: "#2196f3",
    STATE_SUPERCLIP: "#ff2d55",
    STATE_HARVEST: "#ffb300",
    STATE_BRAKE: "#6a6a8a",
}
...
def run(df, meta):
    ...
    car_dot, = ax.plot([], [], "o", markersize=8, color="#ffffff", zorder=10)
    ...
    for i, s in enumerate([STATE_DEPLOY, STATE_SUPERCLIP, STATE_HARVEST, STATE_BRAKE]):
        pct_texts[s] = ax.text(
            0.015, y_start - i * 0.04, "", transform=ax.transAxes,
            color=PALETTE[s], fontsize=9, va="bottom", ha="left", family="monospace",
        )
